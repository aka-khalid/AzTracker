...
# 3. Search Bar
old_search = """<input type="text" id="search-users" onkeyup="filterUsers()" placeholder="Search by ID..." class="w-full bg-gray-900 border border-gray-800 rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-gray-700 transition">"""
new_search = """<input type="text" id="search-users" onkeyup="filterUsers()" placeholder="Search Name, @username or ID..." class="w-full bg-gray-900 border border-gray-800 rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-gray-700 transition">"""
content = content.replace(old_search, new_search)

# 4. fetchAPI response
old_fetch = """                if (res.status === 202) return { status: 'queued' };
                return await res.json();
            } catch (err) {"""
new_fetch = """                if (res.status === 202) return { status: 'queued' };
...
# 6. queue loop in renderTabs
old_queue = """                list.innerHTML = appData.joinQueue.map(u => {
...
new_queue = """                list.innerHTML = appData.joinQueue.map(u => {
                    const time = new Date(u.requested_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
                    let displayName = u.id;
                    if (u.first_name || u.username) {
                        displayName = `${u.first_name || ''} ${u.username ? '(@' + u.username + ')' : ''} - ${u.id}`.trim();
                    }
                    return `
                    <div class="glass rounded-xl p-3 flex justify-between items-center">
                        <div>
                            <div class="font-medium text-sm truncate max-w-[150px]" title="${displayName}">${displayName}</div>
                            <div class="text-xs text-gray-500 mt-0.5">Requested: ${time}</div>
                        </div>"""
...
new_filter = """        function filterUsers() {
            ...
            list.innerHTML = filtered.map(u => {
                ...
                const limitText = isTargetAdmin ? 'Unlimited Items' : `${u.active_items || 0} / ${u.item_limit} Items`;
