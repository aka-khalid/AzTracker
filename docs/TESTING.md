...
| :--- | :--- | :--- | :--- |
| **TC-501** | Queue Pagination (Recurse) | Push a mock payload to `scraper-queue` with `offset: 0` and ensure > 10 items in DB. | `src/workers/scraper_engine.js` processes 10 items, returns `true`, and `src/workers/queue_worker.js` automatically sends a new message to `scraper-queue` with `offset: 10` and a 1-second delay. |
| **TC-502** | Scraper API Outage (Failsafe) | Mock the Amazon Creators API to return 0 items for a batch of >= 5 active ASINs. | `scraper_engine.js` throws an error ("0 items returned"), causing `queue_worker.js` to call `msg.retry()`, aborting the chain and retrying automatically. |
...
| **TC-504** | Out-of-Stock Failsafe | Mock an ASIN missing from Amazon for > 24 hours (all conditions). | The engine updates D1 setting `delisted = 1`, pauses user subscriptions, and pushes a Telegram alert to the `telegram-outbox`. |
...
| Test ID | Feature Target | Diagnostic Vector / Execution Method | Expected Success Criterion |
| :--- | :--- | :--- | :--- |
| **TC-601** | Cutover Initialization | Run `node finalize_cutover.js` and select Development [1]. | Prompts process successfully without crashing and generate a localized 16-byte cryptographically secure webhook token. |
| **TC-602** | Worker Secret Injection | Verify Cloudflare REST API execution. | Verifies the script correctly issues requests to inject `TELEGRAM_BOT_TOKEN`, `AMAZON_CLIENT_ID`, and other required secrets. |
| **TC-603** | Telegram Webhook | Trigger the final health probe sequence in `finalize_cutover.js`. | Telegram API responds with `ok: true` and confirms the webhook was set containing the `secret_token` header payload. |
...
| Test ID | Feature Target | Diagnostic Vector / Execution Method | Expected Success Criterion |
| :--- | :--- | :--- | :--- |
| **TC-701** | Asynchronous Payload Queueing | Trigger a mock price drop in `executeScrapeEngine`. | The engine does NOT block for Telegram. It pushes the payload to `telegram-outbox` with `type: 'telegram_alert_new'` or `telegram_alert_used`. |
...
| **TC-705** | Omnichannel Broadcast | Mock an Exceptional Deal (Z-Score <= -1.5) in the batch results. | The engine appends exactly one broadcast payload to the `telegram-outbox` targeting `TELEGRAM_PUBLIC_CHANNEL_ID` and stamps `last_broadcast_time_ms` in the database. |
...
1. Ensure you are on the correct feature branch (`AzTracker-feature-phase-6.10-modular-decomposition`).
2. To trigger a manual scrape cycle via the CLI, inject a payload into the scraper queue:
   ...
   ```
3. Use `wrangler tail` to monitor the logs. Verify that `src/workers/queue_worker.js` processes the offset, recursively queues the next offset if `hasMore` is true, and safely catches API errors.
