...
| **TC-604** | Worker Secret Injection | Mock the Cloudflare REST API responses for the secrets endpoint. | Verifies the script correctly issues `PUT` requests to the Cloudflare Workers REST API for all 6 required production secrets. |
